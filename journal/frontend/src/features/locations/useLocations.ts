...
import { Location, createLocation, deleteLocation, getLocations, updateLocation } from "../../api/locations";
...
export function useLocations() {    
    const [sortOrder, setSortOrder] = useLocalStorage<Number[]>(LOCATIONS_SORT_STORAGE_KEY, [])
    ...
    const createMutation = useMutation({
        ...
        onSuccess: (result) => {
            queryClient.setQueryData([LOCATIONS_QUERY_KEY], (old: Location[]) => old.concat(result))
        }
    })
    const deleteMutation = useMutation({
        mutationFn: deleteLocation,
        onSuccess: (result, variables) => {
            queryClient.setQueryData([LOCATIONS_QUERY_KEY], (old: Location[]) => old.filter(p => p.id != variables))
        }
    })
